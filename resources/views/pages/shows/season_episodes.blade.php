...
@else
  @section('head_title', stripslashes($series_name).' '.stripslashes($season_info->season_name).' | '.getcong('site_name'))
@endif
     ...
     <div class="video-shows-section vfx-item-ptb tv-season-related-block">
      ...
      <div class="row">
        ...
        <div class="vfx-item-section">
          <h3><a href="{{ URL::to('shows/details/'.$series_slug.'/'.$season_info->series_id) }}" title="{{$series_name}}">{{stripslashes($series_name)}}</a> - {{$season_info->season_name}}</h3>           
        </div>
