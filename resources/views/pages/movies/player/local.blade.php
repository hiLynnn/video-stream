...
<script type="text/javascript">
			FWDEVPUtils.onReady(function(){
				...
				new FWDEVPlayer({		
					...
					stopAtTime:"",
					videoSource:[
						{source:"encrypt:{{base64_encode(URL::to('/'.$movies_info->video_url))}}", label:"default"},
						
						 
						@if($movies_info->video_quality)
						@if($movies_info->video_url_480)
						{source:"encrypt:{{base64_encode(URL::to('/'.$movies_info->video_url_480))}}", label:"480"},
						@endif	
							
						@if($movies_info->video_url_720)	
						{source:"encrypt:{{base64_encode(URL::to('/'.$movies_info->video_url_720))}}", label:"720"},
						@endif	
							
						@if($movies_info->video_url_1080)	
						{source:"encrypt:{{base64_encode(URL::to('/'.$movies_info->video_url_1080))}}", label:"1080"}
						@endif	  
						@endif
					],
