          ...
          <div class="row">
            <div class="col-12">
              <div class="card-box table-responsive">
                ...
                <div class="col-md-3">
                  <a href="{{URL::to('admin/sports/add_video')}}" class="btn btn-success btn-md waves-effect waves-light m-b-20 mt-2 pull-right" data-toggle="tooltip" title="{{trans('words.add_video')}}"><i class="fa fa-plus"></i> {{trans('words.add_video')}}</a>
                </div>
              </div>
              ...
              <div class="row">   
                ...
                <div class="row">
                  @foreach($video_list as $i => $video_data)
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-6" id="card_box_id_{{$video_data->id}}">

                        <!-- Simple card -->
                        <div class="card m-b-20">
                             <div class="checkbox checkbox-success wall_check">
                                <input type="checkbox" name="post_ids[]" id="checkbox<?php echo $i; ?>" value="<?php echo $video_data->id; ?>" class="post_ids">
                                <label for="checkbox<?php echo $i; ?>"></label>
                              </div>

                            @if(isset($video_data->video_image)) <img class="card-img-top thumb-lg img-fluid" src="{{URL::to('/'.$video_data->video_image)}}" alt="{{ stripslashes($video_data->video_title) }}"> @endif
 

                            <div class="card-body p-3">
                                <h4 class="card-title mb-3">{{ stripslashes($video_data->video_title) }}</h4>
                                <a href="{{ url('admin/sports/edit_video/'.$video_data->id) }}" class="btn btn-icon waves-effect waves-light btn-success m-r-5" data-toggle="tooltip" title="{{trans('words.edit')}}"> <i class="fa fa-edit"></i> </a>
                                <a href="#" class="btn btn-icon waves-effect waves-light btn-danger data_remove" data-toggle="tooltip" title="{{trans('words.remove')}}" data-id="{{$video_data->id}}"> <i class="fa fa-remove"></i> </a>
                                <a href="Javascript:void(0);" class="ml-2 fl-right mt-1" data-toggle="tooltip" title="@if($video_data->status==1){{ trans('words.active')}} @else {{trans('words.inactive')}} @endif"><input type="checkbox" name="category_on_off" id="category_on_off" value="1" data-plugin="switchery" data-color="#28a745" data-size="small" class="enable_disable"  data-id="{{$video_data->id}}" @if($video_data->status==1) {{ 'checked' }} @endif/></a>    
                            </div>
                        </div>
...
$("#data_remove_selected").click(function () {  
  ...
  var action_name='sports_delete_selected';
