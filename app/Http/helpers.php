...
if (! function_exists('getImageSizeFromURL')) {
    function getImageSizeFromURL($url) {
        ...
        // Get the image size
        $image_size_info = getimagesizefromstring($image_data);
    
        if ($image_size_info === false) {
            return false; // Unable to get image size
        }
...
if (! function_exists('set_tmdb_language')) {
    ...
    function set_tmdb_language()
    {
        ...
        else if(getcong('default_language')=="es")
        {
            $default_language='es-ES';
        }
        else if(getcong('default_language')=="fr")
        {
            $default_language='fr-FR';
        }
        else if(getcong('default_language')=="pt")
        {
    ...
    {   
        if(Auth::check())
        {
            ...
            if(!empty($plan_info))
            {
                ...
                else if($plan_info->ads_on_off==1)
                {
                    return true;
                }
                else
...
if (! function_exists('user_device_limit_reached')) {
...
if (! function_exists('get_season_trailer')) {
    ...
    function get_season_trailer($series_id, $season_number) {
        ...
        CURLOPT_URL => "https://api.themoviedb.org/3/tv/71912/season/3/videos",
